...
def test_autorization_invalid_email_pass(selenium, incor_email, incor_passw):
    """TК-002, TК-003 "Проверка авторизации пользователя с невалидным email и паролем:
...
def test_elements_of_autorization(selenium):
...
def test_menu_type_autorization(selenium):
    """TК-005 Проверка названия вкладок в меню выбора типа авторизации."""
    try:
        page = AuthPage(selenium)
        ...
        print('Ошибка в имени вкладки Меню типа аутентификации')


def test_menu_of_type_active_autorization(selenium):
    """TК-006 Проверка выбора вкладки по умолчанию в Меню выбора типа авторизации."""
    page = AuthPage(selenium)
...
def test_page_logo_registration(selenium):
    """TК-010 Проверка блока со слоганом компании на странице "Регистрация"."""
    try:
...
def test_registration_valid_data(selenium):
    """TК-013 Проверка Регистрации пользователя с валидными данными "Имя" и "Фамилия", кириллица."""
    page_reg = RegistrPage(selenium)
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    ...
    буквы кириллицы в количестве 5 ; 8 ; 13 ; 24 ; 32 ."""
    page_reg = RegistrPage(selenium)
...
def test_first_name_invalid_data(selenium, invalid_first_name):
    ...
    пустое значение;
    буквы кириллицы в количестве 1 ; 100 ; 257 ;
    латинские буквы; китайские иероглифы; спецсимволы; числа."""
    page_reg = RegistrPage(selenium)
...
# python3 -m pytest -v --driver Chrome --driver-path chromedriver tests/test.py